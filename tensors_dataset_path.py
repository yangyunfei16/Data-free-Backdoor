...
class TensorDatasetPath(Dataset):
    ...
    def __getitem__(self, index):
        if self.data_name == "VGGFace":
            img_path = self.data_tensor[index]
            # print(img_path)
        ...
        if ((self.mode=='train' and random.random()<self.poison_ratio) or (self.mode=='test' and self.test_poisoned=='True')):
            ...
            if self.data_name == "VGGFace":
                ...
                # img[start_h:(start_h+60),start_w:(start_w+60),:] = (1-self.opacity) * img[start_h:(start_h+60),start_w:(start_w+60),:] + self.opacity * trigger[start_h:(start_h+60),start_w:(start_w+60),:]
